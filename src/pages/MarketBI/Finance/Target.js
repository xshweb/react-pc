...
const FormModal = (props) => {
  ...
  const submitData = async (param) => {
    ...
    try {
      const result = await api.stimateAdd(params);
  ...
  const checkNum = (_, value) => {
    ...
    return Promise.reject(new Error('输入不合法，请输入正数!'));
  };
...
const columns = [
  {
    title: '年目标发货盒数(盒)',
    dataIndex: 'annualTargetDelivery',
    key: 'annualTargetDelivery'
  },
  {
    title: '年目标回款金额(元)',
...
const TargetData = () => {
  ...
  useEffect(() => {
    ...
    const gotData = async () => {
      try {
        const result = await api.stimateList();
        if (!didCancel) {
          if (result.resultCode === 0) {
